...
# /l command — reply-based file tagging with 10 tools.
#
# Supported tools:
...
@StreamBot.on_message(filters.command("l") & filters.private)
async def leech_command(client: Client, m: Message):
    ...
    done_cancel_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Done", callback_data="leech_done_confirm")],
        [InlineKeyboardButton("❌ Cancel Task", callback_data="leech_done_cancel")],
    ])
    summary = _build_task_summary(task)
    await client.send_message(
        m.chat.id,
        f"{summary}\n\nPress **Done** to start processing:",
        reply_markup=done_cancel_kb,
    )

    # Wait for Done or Cancel via callback
    while task.status == TaskStatus.WAITING_FOR_DONE:
        if task.cancel_event.is_set():
            return
        await asyncio.sleep(0.5)
    if task.status == TaskStatus.CANCELLED:
    ...
    # ── Upload ──
    # RV always uses batch upload (multi-audio → multiple outputs from 1 file)
    use_batch = (tool in ("cv", "wv", "ev", "cut") and file_count > 1) or tool == "rv"
    if use_batch:
    ...
    if upload_ok:
        ...
        # Dump task log to BIN_CHANNEL
        await _dump_task_log(client, task, downloaded_files)
    cleanup_task_files(task)
...
@StreamBot.on_callback_query(filters.regex(r"^leech_done_"), group=11)
async def done_cancel_callback(client: Client, cb: CallbackQuery):
    """Handle Done / Cancel buttons in the done-confirmation step."""
    user_id = cb.from_user.id
    task = None
    for t in ACTIVE_LEECH_TASKS.values():
        if t.user_id == user_id and t.status == TaskStatus.WAITING_FOR_DONE:
            task = t
            break
    if not task:
    ...
    if cb.data == "leech_done_confirm":
        task.status = TaskStatus.DOWNLOADING  # breaks the polling loop
        await cb.answer("Starting...")
        try:
            await cb.message.edit_text(f"✅ **Confirmed** — starting downloads for task `{task.task_id}`")
        except Exception:
            pass
...
async def help_command(client: Client, m: Message):
    help_text = (
        "📖 **Bot Help — Available Commands & Tools**\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "**📌 Commands:**\n"
        "`/l` — Media processing (reply to first file)\n"
        "`/cancel <task_id>` — Cancel a running task\n"
        "`/help` — Show this help message\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "**🛠 How to use /l:**\n"
        "1️⃣ Send your media files to the bot\n"
        "2️⃣ Swipe to the **first** file\n"
        "3️⃣ Reply with: `/l -i <count> -m <output_name> -<tool>`\n\n"
        "**Flags:**\n"
        "`-i` — Number of files (e.g. `-i 5`)\n"
        "`-m` — Output filename (e.g. `-m output.mp4`)\n"
        "`-start` — Start timestamp for trim/cut\n"
        "`-end` — End timestamp for trim/cut\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "**🔧 Available Tools (10):**\n\n"
        "**Merge Tools:**\n"
        "`-vt` **Video + Video** — Concatenate videos\n"
        "`-va` **Video + Audio** — Add audio track(s)\n"
        "`-aa` **Audio + Audio** — Concatenate audio\n"
        "`-vs` **Video + Subtitle** — Hardcode or soft subs\n\n"
        "**Processing Tools:**\n"
        "`-cv` **Compress Video** — 5 modes (quality/size/CRF)\n"
        "`-wv` **Watermark Video** — Text/image, 8 animations\n\n"
        "**Trim & Cut:**\n"
        "`-tv` **Trim Video** — Keep segment (start→end)\n"
        "`-cut` **Cut Video** — Remove segment, stitch rest\n\n"
        "**Stream Extraction:**\n"
        "`-rv` **Extract Audio** — Remove video, keep audio\n"
        "  ↳ Supports multi-audio (all tracks extracted)\n"
        "  ↳ Formats: MP3, AAC, WAV, Keep Original\n"
        "`-ev` **Extract Video** — Remove audio, keep video\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "**📝 Examples:**\n"
        "`/l -i 2 -m merged.mp4 -vt`\n"
        "`/l -i 2 -m output.mp4 -va`\n"
        "`/l -i 1 -m trimmed.mp4 -tv -start 00:01:00 -end 00:02:30`\n"
        "`/l -i 1 -m cut.mp4 -cut -start 00:05:00 -end 00:07:00`\n"
        "`/l -i 3 -m compressed -cv`\n"
        "`/l -i 1 -m audio -rv`\n"
        "`/l -i 1 -m video_only -ev`\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "**⚡ Features:**\n"
        "• Batch processing — apply same tool to multiple files\n"
        "• Staggered downloads — parallel with rate limiting\n"
        "• Cancel anytime — `/cancel <task_id>`\n"
        "• Quality preservation — uses `-c copy` when possible\n"
    )
    await m.reply_text(help_text, quote=True)


# ═══════════════════════════════════════════════════════════════
#  TASK DUMP LOG — Forward to BIN_CHANNEL
# ═══════════════════════════════════════════════════════════════

async def _dump_task_log(client: Client, task: LeechTask, downloaded_files: list):
    """Forward task completion log + input/output files to BIN_CHANNEL."""
    try:
        duration = int(time.time() - task.created_at)
        job_hash = hashlib.md5(f"{task.task_id}{task.created_at}".encode()).hexdigest()[:8]
        tool_label = AVAILABLE_TOOLS.get(task.selected_tool, task.selected_tool)
        ...
        for idx, path, msg in downloaded_files:
            ...
            input_sizes.append(fsize)

        total_input_size = sum(input_sizes)
        input_display = ", ".join(input_names[:3])
        if len(input_names) > 3:
            input_display += f" (+{len(input_names) - 3} more)"

        # Output files info
        output_files = []
        total_output_size = 0
        if os.path.exists(task.work_dir):
            for f in os.listdir(task.work_dir):
                fp = os.path.join(task.work_dir, f)
                if os.path.isfile(fp) and not f.startswith("."):
                    try:
                        sz = os.path.getsize(fp)
                    except Exception:
                        sz = 0
                    output_files.append((f, fp, sz))
        ...
        log_text = (
            ...
            f"👤 **User:** `{task.user_id}`\n\n"
            f"⚙ **Action:** {tool_label}\n"
            f"📂 **Input:** {input_display} ({humanbytes(total_input_size)})\n"
            f"📤 **Output:** {task.output_name} ({humanbytes(total_output_size)})\n\n"
            f"⏱ **Duration:** {duration}s\n"
            f"📅 {now_utc}\n\n"
            f"#{task.selected_tool} #completed #task_{task.task_id} #uid_{task.user_id}"
        )

        # Send log text to BIN_CHANNEL
        log_msg = await client.send_message(Var.BIN_CHANNEL, log_text)

        # Forward input files to BIN_CHANNEL
        for idx, path, msg in downloaded_files:
            try:
                await msg.forward(chat_id=Var.BIN_CHANNEL)
            except Exception as e:
                logger.warning("Failed to forward input file: %s", e)

        # Upload output files to BIN_CHANNEL
        for fname, fp, sz in output_files:
            try:
                ext = os.path.splitext(fname)[1].lower()
                caption = f"📤 **Output** | Task: `{task.task_id}`\n📁 `{fname}` ({humanbytes(sz)})"
                if ext in (".mp4", ".mkv", ".avi", ".webm", ".mov", ".flv", ".ts"):
                    await client.send_video(Var.BIN_CHANNEL, fp, caption=caption, file_name=fname)
                elif ext in (".mp3", ".aac", ".ogg", ".flac", ".wav", ".m4a", ".opus", ".mka"):
                    await client.send_audio(Var.BIN_CHANNEL, fp, caption=caption, file_name=fname)
                else:
                    await client.send_document(Var.BIN_CHANNEL, fp, caption=caption, file_name=fname)
            except Exception as e:
                logger.warning("Failed to upload output %s to BIN_CHANNEL: %s", fname, e)

    except Exception as e:
        logger.error("Task dump log failed for %s: %s", task.task_id, e)
