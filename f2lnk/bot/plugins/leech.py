...
async def _process_video_audio(client, task, downloaded_files, status_msg) -> bool:
    ...
    # Copy video stream (no re-encode), re-encode each audio to AAC
    # so that each audio track is independently encoded in the container.
    # Using -c copy for all streams causes MP4 to share data across
    # audio tracks, resulting in only the last audio playing on all tracks.
    cmd.extend(["-c:v", "copy", "-c:a", "aac", "-b:a", "192k"])

    # Use MKV container if output is .mkv, otherwise default
    # MKV/MP4 both support multiple audio streams
