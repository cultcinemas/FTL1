...
def _parse_leech_args(text: str):
    """Parse /leech flags. Returns (file_count, output_name, suggested_tool)."""
    file_count = None
    output_name = None
    ...
    # Check for tool flags
    for flag, tool_key in FLAG_TO_TOOL.items():
        if flag in text:
...
async def leech_command(client: Client, m: Message):
    ...
    if tool == "vt":
        ok = await process_video_video(task, downloaded_files, status_msg)
    elif tool == "va":
        ok = await process_video_audio(client, task, downloaded_files, status_msg)
    elif tool == "aa":
        ok = await process_audio_audio(task, downloaded_files, status_msg)
    elif tool == "vs":
        ok = await process_video_subtitle(client, task, downloaded_files, status_msg)
    elif tool == "cv":
        ok = await process_compress_video(client, task, downloaded_files, status_msg)
    elif tool == "wv":
        ok = await process_watermark_video(client, task, downloaded_files, status_msg)
    else:
        task.status = TaskStatus.FAILED
        await status_msg.edit_text(f"❌ Unknown tool: `{tool}`")
        cleanup_task_files(task)
        remove_task(task_id)
        return
    ...
    # ── Upload ──
    if tool in ("cv", "wv") and file_count > 1:
